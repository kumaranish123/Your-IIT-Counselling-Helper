...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $institute_name = $row['COL 1'];
            $closing_rank = $row['COL 7'];
            $year = $row['COL 8'];

            // if the program doesn't exist in the array yet, add it
            if (!isset($institutes[$institute_name])) {
                $institutes[$institute_name] = array();
            }

            // add the closing rank for this year to the program's array
            $institutes[$institute_name][$year] = $closing_rank;
        }
    ...
    <script>
        ...
        var data = {
            ...
            datasets: [
                <?php foreach ($institutes as $institute_name => $closing_ranks) { ?> {
                        label: '<?php echo $institute_name; ?>',
        ...
        var res = <?php echo json_encode($institutes); ?>;
